...
var Cluster = module.exports = function(host, port, login) {
    ...
    this.socket = net.connect(port, host, function() {
        log.info("Connected to the DX cluster");
        self.connected = true;
    });
    ...
    this.socket.on("error", function(err) {
        log.warn("Failed to connect to the DX cluster");
    });

    this.socket.on("close", function(data) {
        log.warn("Disconnected from the dx cluster");
...
Cluster.prototype.parse = function(data) {
    if (data.match(/^login/i)) {
        ...
        var time = time.substring(0,2) + ":" + time.substring(2,4);
