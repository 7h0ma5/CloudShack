...
var Dxcc = function(callback) {
    ...
    if (!fs.existsSync("cty.dat")) {
        this.download(callback);
    }
    else {
        console.log("using cached cty.dat");
        this.parse(callback);
    }
};

Dxcc.prototype.parse = function(callback) {
...
Dxcc.prototype.download = function(callback) {
    console.log("downloading cty.dat...");
    var self = this;
    http.get(cty_url, function(res) {
        var file = fs.createWriteStream("cty.dat");
        var pipe = res.pipe(file);
        pipe.on("close", function() {
            console.log("cty.dat downloaded");
            self.parse(callback);
        });
    }).on("error", function(err) {
        console.log("unable to download cty.dat");
    });
};
