export function coord_distance(A: [number, number], B: [number, number]) : number {
...
export function coord_bearing(A: [number, number], B: [number, number]) : number {
    var φA = A[0] * Math.PI / 180;
    var φB = B[0] * Math.PI / 180;
    var Δλ = (B[1] - A[1]) * Math.PI / 180;
...
export function validate_grid(grid: string) : boolean {
    return /^[A-Za-z]{2}[0-9]{2}([A-Za-z]{2})?$/.test(grid);
}

export function grid_to_coord(grid: string) : [number, number] {
...
export function coord_to_grid(coord: [number, number]) {
    var lat = coord[0] + 90,
        lon = coord[1] + 180,
        grid = "";
